...
class Solution {
private:
    void dfs(int idx, vector<int>& candidates, int target, vector<vector<int>>& res, vector<int>& temp) {
        ...
        // 直接跳过, 跳到下一个数的 dfs 部分
        dfs(idx + 1, candidates, target, res, temp);
        
        // 判断还能减少 candidates[idx]
        if (target - candidates[idx] >= 0) {
            temp.emplace_back(candidates[idx]);
            dfs(idx, candidates, target - candidates[idx], res, temp);
            temp.pop_back();            // 回溯
        }
    }

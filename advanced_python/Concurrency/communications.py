...
def test_event(n):
    ...
    event.clear()       # 重置 event，使得所有该 event 事件都处于待命状态
    ...
    print('唤醒所有线程 ...')
    event.set()                 # 发送 event 指令，使所有设置该 event 事件的线程执行
...
# 队列的使用上
# 从一个线程向另一个线程发送数据最安全的方式是使用 queue 库中的队列。创建一个被多个线程共享的队列对象，
# 通过 put() 和 get() 方法向队列发送和获取元素。
